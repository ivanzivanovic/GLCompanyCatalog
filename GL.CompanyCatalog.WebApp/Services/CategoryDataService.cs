...
namespace GL.CompanyCatalog.WebApp.Services
{
    public class CategoryDataService : BaseDataService, ICategoryDataService
    {
        ...
        public async Task<List<CategoryCompaniesViewModel>> GetAllCategoriesWithCompanies(bool includeHistory)
        {
            var allCategories = await _client.GetCategoriesWithCompaniesAsync(includeHistory);
            var mappedCategories = _mapper.Map<ICollection<CategoryCompaniesViewModel>>(allCategories);
            return mappedCategories.ToList();
        }
